...
export default function Logo({ size = 34, withWordmark = true, className = '' }) {
  ...
  const gradId = `ctGrad-${uniqueId.replace(/:/g, '')}`;
  const gradSoftId = `ctGradSoft-${uniqueId.replace(/:/g, '')}`;

  return (
    <span className={className} style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
      <svg width={size} height={size} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id={gradId} x1="4" y1="42" x2="44" y2="6" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor="#2f8dff" />
            <stop offset="100%" stopColor="#5b6ee8" />
          </linearGradient>
          <linearGradient id={gradSoftId} x1="4" y1="4" x2="44" y2="44" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor="#22d3ee" stopOpacity="0.55" />
            <stop offset="100%" stopColor="#5b6ee8" stopOpacity="0.15" />
          </linearGradient>
        </defs>

        <rect x="1" y="1" width="46" height="46" rx="13" fill="#0d0d0d" stroke={`url(#${gradSoftId})`} strokeWidth="1.4" />

        {/* Skyline steps — three ascending bars reading as buildings */}
        <rect x="10" y="26" width="6" height="12" rx="1.4" fill={`url(#${gradId})`} opacity="0.85" />
        <rect x="18" y="19" width="6" height="19" rx="1.4" fill={`url(#${gradId})`} />
        <rect x="26" y="12" width="6" height="26" rx="1.4" fill={`url(#${gradId})`} opacity="0.85" />
